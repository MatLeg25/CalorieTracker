...
fun NutrientsBar(
    ...
    LaunchedEffect(key1 = carbs) {
        carbWidthRatio.animateTo(
            targetValue = ((carbs * Nutrients.KCAL_IN_1G_CARBS) / calorieGoal)
        )
    }
    LaunchedEffect(key1 = protein) {
        proteinWidthRatio.animateTo(
            targetValue = ((carbs * Nutrients.KCAL_IN_1G_PROTEIN) / calorieGoal)
        )
    }
    LaunchedEffect(key1 = fat) {
        fatWidthRatio.animateTo(
            targetValue = ((carbs * Nutrients.KCAL_IN_1G_FAT) / calorieGoal)
        )
    }
